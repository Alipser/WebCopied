  ...
  (function () {
    ...
    function _ins() {
      ...
      this.init = function (site_id) {
        ...
        if (
          clicky_custom.getHighEntropyValues &&
      ...
      this.custom_data = function () {
        ...
        if (clicky_custom.visitor) {
          for (var i in clicky_custom.visitor) {
            if (
              clicky_custom.visitor.hasOwnProperty &&
              clicky_custom.visitor.hasOwnProperty(i)
            )
      ...
      this.set_referrer = function () {
        var r =
          clicky_custom.track_iframe || !top.document.referrer
            ? document.referrer
      ...
      this.pageview = function (only_once) {
        ...
        _self.beacon(
          ...
          "&href=" +
            ...
            _self.enc(
              clicky_custom.title ||
                window.clicky_page_title ||
                (clicky_custom.track_iframe || !top.document.title
                  ? document.title
                  : top.document.title)
            ) +
      ...
      this.get_href = function (enc) {
        var href =
          ...
          (clicky_custom.track_iframe
            ...
            : top.location.pathname + top.location.search);
      ...
      this.facebook_is_lame = function (href) {
        return (
          href &&
          href.match &&
          href.match(/fb_xd_fragment|fb_xd_bust|fbc_channel/i)
        );
      };
      this.heatmap_xy = function (e) {
        ...
        if (!clicky_custom.heatmap_disable)
          _self.beacon(
            "heatmap",
            "&heatmap[]=" + _self.enc(href) + "|" + x + "|" + y + "|" + w.w
          );
      };
      ...
      this.heatmap_data = function (date, sub, subitem) {
        ...
        _self.inject(
          "//clicky.com/ajax/onsitestats/heatmap?" +
            ...
            "&h=" +
            wh.h +
            (location.hash.match(/^#_heatmap/)
              ? location.hash.replace(/^#_heatmap/, "")
              : "") +
            (date ? "&date=" + date : "") +
            (sub ? "&sub=" + sub : "") +
      ...
      this.beacon = function (type, q, called_by_pageview) {
          ...
          if (clicky_custom.goal) {
            if (typeof clicky_custom.goal == "object") {
              for (var i in clicky_custom.goal) {
                if (
                  clicky_custom.goal.hasOwnProperty &&
                  clicky_custom.goal.hasOwnProperty(i)
                )
                  goal +=
                    "&goal[" +
                    _self.enc(i) +
                    "]=" +
                    _self.enc(clicky_custom.goal[i]);
              }
            } else {
          ...
          if (clicky_custom.split) {
            for (var i in clicky_custom["split"]) {
              if (
                ...
                if (
                  i == "goal" &&
                  typeof clicky_custom["split"].goal == "object"
                ) {
                  for (
                    var j = 0, l = clicky_custom["split"].goal.length;
                    j < l;
                    j++
                  ) {
                  ...
                  split +=
                    ...
                    _self.enc(i) +
                    "]=" +
                    _self.enc(clicky_custom.split[i]);
              }
            }
            clicky_custom.split = "";
          }
        ...
        for (
          var site_id_index = 0;
          site_id_index < site_ids.length;
          site_id_index++
        ) {
          var site_id = site_ids[site_id_index],
            hm =
              _self.get_cookie("_heatmaps_g2g_" + site_id) ||
              window["_heatmaps_g2g_" + site_id] ||
              "";
          ...
          if (
            called_by_pageview &&
            type == "pageview" &&
            _self.is_pageview_fired(site_id)
          )
            continue;
          _self.inject(
            _self.domain +
              ...
              (_self.he_platform ? "&hep=" + _self.he_platform : "") +
              (_self.he_model ? "&hem=" + _self.he_model : "") +
              "&ck=" +
              (navigator.cookieEnabled && !clicky_custom.cookies_disable
                ? 1
                : 0) +
              (jsuid ? "&jsuid=" + jsuid : "") +
              (hm ? "&hm=" + hm : "") +
      ...
      this.ping_start = function () {
        if (
          ...
          (clicky_custom.timeout &&
            (clicky_custom.timeout < 5 || clicky_custom.timeout > 240))
        )
          return;
        _self.pinging = 1;
        _self.ps_stop = clicky_custom.timeout
      ...
      this.get_cookie = function (name) {
        if (
          ...
          name.match(/^_(custom|utm|referrer)/)
        )
          return "";
      ...
      this.set_cookie = function (name, value, expires, force) {
        if (
          ...
          return false;
        var maxage =
          expires == "session" ? "" : ";max-age=" + (expires || 86400 * 365);
      ...
      this.advanced = function () {
        ...
        var is_download = new RegExp(
          "\\.(7z|aac|apk|avi|cab|csv|dmg|doc(x|m|b)?|epub|exe|flv|gif|gz|jpe?g|js|m4a|mp(3|4|e?g)|mobi|mov|msi|ods|pdf|phps|png|ppt(x|m|b)?|rar|rtf|sea|sit|svgz?|tar|torrent|txt|vcf|web(m|p)|wma|wmv|xls(x|m|b)?|xml|zip)$",
          "i"
        );
      ...
      this.outbound_pattern_match = function (href, pattern) {
        return RegExp(pattern.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&")).test(
          href
        );
      };
